...
async def anketa_handler(msg: Message, state: FSMContext):
    ...
    markup = InlineKeyboardMarkup(inline_keyboard=[[
        InlineKeyboardButton(text='Отмена', calback_data = 'cancel_anreta')]])
    await msg.answer('Введите ваше имя', reply_markup=markup)

@router.callback_query(F.data == "cancel_anketa")
async def cancel_handler(calback_query: CallbackQuery, state: FSMContext):
    await state.clear()
    await calback_query.message.answer('Регистрация отменена')
...
async def set_name_by_anketa_handler(msg: Message, state: FSMContext):
    ...
    markup = InlineKeyboardMarkup(inline_keyboard=[[
        InlineKeyboardButton(text='Назад', calback_data = 'set_name_anketa'),
        InlineKeyboardButton(text='Отмена', calback_data = 'cancel_anketa')]])
    await msg.answer('Введите ваш возраст', reply_markup=markup)

@router.callback_query(F.data == "set_name_anketa")
async def set_name_by_anketa_handler(callback_query: CallbackQuery, state: FSMContext):
...
async def set_age_anketa_handler(calback_query: CallbackQuery, state: FSMContext):
    await state.set_state(Anketa.age)
    markup = InlineKeyboardMarkup(inline_keyboard=[[
        InlineKeyboardButton(text='Назад', calback_data = 'set_name_anketa'),
        InlineKeyboardButton(text='Отмена', calback_data = 'cancel_anketa')]])
    await calback_query.answer('Введите ваш возраст', reply_markup= markup)
